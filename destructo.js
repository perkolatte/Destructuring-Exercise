...
// TODO:
// Destructo has found a map with coordinates marked on it.
...
// TODO:
// The map reveals several locations, but
// only the first two are significant for the quest.
// Use object destructuring with the rest parameter
// to isolate the `first` and `second` locations from the `locations` object,
// capturing the rest in a variable called `remaining`.
...
// TODO:
// To open the Mysterious Door, a sequence is required, which might be incomplete.
// Use object destructuring to assign default values
// to ensure the door opens even if part of the code is missing.
// Make sure the `middle` defaults to one of the values from the `remaining` variable above
// if not provided.
...
const {
  upper,
  lower,
  middle = remaining[Math.random() < 0.5 ? "third" : "fourth"],
} = doorCode;
...
// TODO:
// The guardian of an ancient library speaks in riddles.
// Use object destructuring to rename `ancientWord` to `translation` in the `riddle` object.
// Print the translation of the riddle.

const { modernWord: ancientWord } = riddle;
console.log(`Translation: ${ancientWord}`);

/* Task 5: The Array of Elements */
const elements = ["Fire", "Water", "Earth", "Air"];
// TODO:
// Inside the library,
// Destructo discovers an array that represents the elements needed to decipher the next clue.
...
// TODO:
// Crossing the River of Reflections requires skipping certain stones.
...
// TODO:
// The Cave of Shadows hides more than it reveals.
...
// TODO:
// Destructo needs to decode ancient directions to continue his quest.
...
// TODO:
// Destructo finds an ancient scroll with a potion recipe,
// but some ingredients are missing.
// Write a function `mixPotion`
// that uses defaults "Water" and "Fireflower" for `ingredient1` and `ingredient2` if they are not specified and
// print those mixings.
...
mixPotion({ ingredient1: "Dragon's Blood" });

/* Task 10: The Array Spell */
// TODO:
// At the gates of an ancient library,
// Destructo must cast a spell with the first two ingredients from a list given to him by a wise owl.
// Create a function `castSpell` that uses array destructuring to access these ingredients from an array and
// print the spell casting.
...
castSpell(["Phoenix Feather", "Dragon Scale", "Mermaid Tear"]);

/* Task 11: The Nested Secret */
const nestedSecret = { outer: { inner: "The Final Key" } };
// TODO:
// Behind the final door lies a nested artifact containing the ultimate clue.
...
// two mystical stones control the treasure's safeguard.
